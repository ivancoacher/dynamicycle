---
id: 360041411132
title: "Understanding Smart Send Time results"
...
Klaviyo uses a 2-part process to pick the optimal send time.
...
|  |  |
| --- | --- |
| ****Number of profiles**** | ****Campaigns required**** |
| 12,000–17,999 | 4–5 |
| 18,000–23,999 | 3–4 |
| 24,000–47,999 | 2–3 |
| 48,000–71,999 | 1–2 |
...
After the exploratory send(s), Klaviyo will determine the best time for the highest engagement and will perform a focused send. In this stage, one email will send at a given time, one will have sent two hours prior, and one will send two hours after in order to validate the selected time.

## Understand Smart Send Time data

1. Navigate to the ****Campaigns**** tab.
2. Click ****Options****.
3. Click ****View Smart Send Time Report****.
   ![view sst report.jpg](https://klaviyo.zendesk.com/hc/article_attachments/28704477104411)

Here, you can view data from your Smart Send Time campaigns. **Smart Send Time activity** shows when you sent an exploratory or focused campaign, if the focused time has changed, and more. In addition, there are graphs that show the open rate per hour, opens and conversions per hour, and expected open rate lift.

Note that the open rates represented in these graphs are only inclusive of campaigns sent as part of the Smart Send Time test selected in the dropdown menu at the top of the page.

### Open rate per hour

The **Open rate per hour graph** shows you the percentage of people who opened a Smart Send Time campaign out of those who received your email at a certain time. Exploratory sends, for example, send a campaign to different recipients over a 24-hour period. This graph shows the percentage of people who opened the email for each hour. For instance, 1 p.m. data shows the percentage of people who opened the email who received it at 1 p.m. in their local timezone.

If a focused send has a much higher open rate than any of your other sends, we only analyze the relative change between hours which is reflected in this graph. This means that if a single campaign does very well, we will not spike the hours for that focused send because a single campaign performed better than average.

![The charts on a Smart Send Time results page](https://klaviyo.zendesk.com/hc/article_attachments/28704485218971)

This graph only applies to campaigns sent using the Smart Send Time test selected under **Test Name**. It does not include your historical campaigns, ones sent without Smart Send Time, or ones sent as part of another Smart Send Time test, as differences in subject lines and other factors could skew the results. Due to this, the rates may look different than your typical open rates.

### Opens and conversions per hour

The **Open rate and Placed Order Rate per Hour** graph shows the number of opens and conversions (e.g., Placed Order events) from your exploratory sends. As with the Open Rate per Hour graph, all times are based on when the email was sent. If someone received the email at 8 a.m. but didn’t place an order until 9 a.m., the placed order event will still be attributed to the 8 o’clock email.
...
![Smart Send Time results by hour](https://klaviyo.zendesk.com/hc/article_attachments/28704485214235)
...
![The expected average lift in open rate when applying your Smart Send Time learnings](https://klaviyo.zendesk.com/hc/article_attachments/28704477099803)
...
- [How to get started with Smart Send Time](https://help.klaviyo.com/hc/en-us/articles/360029794371-Getting-Started-with-Smart-Send-Time)
- [How to create and send an email campaign](https://help.klaviyo.com/hc/en-us/articles/115005054847-Create-and-Send-an-Email-Campaign)